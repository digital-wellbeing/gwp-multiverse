...
specs_reyear <- expand_grid(
  ...
  mutate(
    formula = str_glue(
      "{y} ~ {x} + {covariate} + ({x} | country) + ({x} | year) + ({x} | country:year)"
    )
  )
```
...
specs_gwb <- specs %>% filter(str_detect(y, "gwb_"))
specs <- specs %>% filter(!str_detect(y, "gwb_"))

n_specs_main <- specs %>% 
  filter(term == x) %>% 
  nrow()
n_specs_gwb <- specs_gwb %>% 
  filter(term == x) %>% 
  nrow()
n_specs_all <- n_specs_main + n_specs_gwb
...
p2 <- mv %>% 
  ...
  geom_point(
    shape = 20, 
    size = .1, 
    position = position_quasirandom(groupOnX = FALSE, width = .2)
  ) +
  facet_grid(
    cols = vars(y),
    rows = vars(name), 
